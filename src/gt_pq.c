...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	if(!(runq->uthread_mask)) { /* No jobs in active. switch runqueue */
        assert(!runq->uthread_tot);
			if(sched_fag==1 &&fag==1)
			{
                int i;
                kthread_context_t *kr;
                for (i = 0; i < 4; i++) {
                    if (i == kthread_cpu_map[i]->cpu_apic_id) continue;
                    else {
                        gt_spin_unlock(&(kthread_runq->kthread_runqlock));
                        kr = kthread_cpu_map[i];
							gt_spin_lock(&(kr->krunqueue.kthread_runqlock));
							if(kr->krunqueue.active_runq->uthread_mask) {
								uprio = LOWEST_BIT_SET(kr->krunqueue.active_runq->uthread_mask);
								prioq = &(kr->krunqueue.active_runq->prio_array[uprio]);
								...
								gt_spin_unlock(&(kr->krunqueue.kthread_runqlock));
						        gt_spin_lock(&(kthread_runq->kthread_runqlock));
								return (u_obj);
							}
